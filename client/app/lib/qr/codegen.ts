...
namespace qrcodegen {
  ...
  export class QrCode {
    ...
    public static encodeSegments(
      ...
      for (version = minVersion; ; version++) {
        const dataCapacityBits: int =
          QrCode.getNumDataCodewords(version, QrEcl) * 8; // Number of data bits available
      ...
      const dataCapacityBits: int =
        QrCode.getNumDataCodewords(version, QrEcl) * 8;
      ...
      return new QrCode(version, QrEcl, dataCodewords, mask);
    }
    ...
    private finderPenaltyTerminateAndCount(
      ...
      let currRunLength = currentRunLength;
      if (currentRunColor) {
        // Terminate dark run
        this.finderPenaltyAddHistory(currRunLength, runHistory);
        currRunLength = 0;
      }
      currRunLength += this.size; // Add light border to final run
      this.finderPenaltyAddHistory(currRunLength, runHistory);
    ...
    private finderPenaltyAddHistory(
      ...
      let currRunLength = currentRunLength;
      if (runHistory[0] === 0) currRunLength += this.size; // Add light border to initial run
      runHistory.pop();
  ...
  export class QrSegment {
    ...
    private static toUtf8ByteArray(str: string): Array<byte> {
      ...
      for (let i = 0; i < encodedStr.length; i++) {
        if (encodedStr.charAt(i) !== '%') result.push(encodedStr.charCodeAt(i));
        else {
          result.push(parseInt(encodedStr.slice(i + 1, 2), 16));
          i += 2;
        }
      }
      return result;
